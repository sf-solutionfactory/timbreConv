...
namespace ZWSSMART4
{
    public class CONSUM_WS4
    {
        public RESULT consum_ws4(
          string _user,
          string _password,
          string _syst,
          string _option,
          ...
          string motivo, string folioSustitucion = null)//ADD SF RSG 01.06.2022)
        {
            ...
            {
                case "1":
                    CONSUM_WS4.TimbrarV1(empty, _url, _user, _password);
                    goto case "6";
                case "2":
                    CONSUM_WS4.TimbrarV2(empty, _url, _user, _password);
                    goto case "6";
                case "3":
                    CONSUM_WS4.TimbrarV3(empty, _url, _user, _password);
                    goto case "6";
                case "4":
                    string xmlInfo = xml_in;
                    if (str == "D")
                        xmlInfo = CONSUM_WS4.GetXmlFile(xml_in);
                    result = CONSUM_WS4.TimbrarV4(xmlInfo, _url, _user, _password);
                    if (str == "D")
                    {
                        try
                        {
                        ...
                        goto case "6";
                case "5":
                    CONSUM_WS4.Autenticacion(_url, _user, _password);
                    goto case "6";
                case "6":
                case "7":
                case "s":
          ...
          string motivo, string folioSustitucion = null)//ADD SF RSG 01.06.2022)
        {
            ...
            CancelationResponse cancelationResponse = new SW.Services.Cancelation.Cancelation(_url, _user, _password).CancelarByCSD(base64String2, base64String1, rfc, password, uuid, motivo, folioSustitucion);//ADD SF RSG 01.06.2022)
